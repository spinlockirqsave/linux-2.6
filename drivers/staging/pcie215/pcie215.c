 ...
 *
 *	3. Enables generation of the interrupts by Altera Cyclone fpga of PCIe215
 *
 ...
 *		if the wake up has been caused by signal.
 *
 *	The pcie215/test directory contains 2 examples.
 */
...
#define PCIE215_DRV_AUTHOR	"Piotr Gregor <piotrgregor@rsyncme.org>"
...
#define PCIE215_LLIER	0x50				/* In/Out	| 32	| Low-level Interrupt Enable Register	*/
 ...
 */
#define PCIE215_IRQ_SRC_MASK	(0x0f)			/* bitmask of valid IRQ triggers */
...
#define PCIE215_IRQ_SRC_CTRZ1_OUT1_OP	BIT(4)		/* for onboard timers, pin 55, not used	*/
#define PCIE215_IRQ_SRC_CTRZ2_OUT1_OP	BIT(5)		/* for onboard timers, pin 58, not used	*/
 ...
 * Bit flags.
 */
#define	PCIE215_FLAGS_IRQ			1			/* 1 if IRQ has been triggered on one of the pins some task is waitnig for, 0 otherwise */
...
MODULE_PARM_DESC(triggers, "This bit mask may be passed to insmod "
		...
		"module load time (interrupt triggering is "
...
struct pcie215 {
	...
	unsigned long	lcr;			/* Local Bus configuration register space, used to access LLIER */
	void __iomem	*ba_iomem;		/* I/O remapped memory of BA					*/
	void __iomem	*lcr_iomem;		/* I/O remapped memory of LCR					*/
	wait_queue_head_t	wait_queue;	/* Queue of the blocked processes waiting for external interrupt*/
	unsigned long		flags;
	__u8			triggers;
	u8			ppi_8255_config;/* Configuration of PPI, stored for the reference only		*/
};
 ...
 *			Enabling new pins _adds_ them to the current mask
 *			(history is kept).
 ...
 *			allowed for interrupt triggering. Disabling pins _subtructs_
 *			them from the current mask. Previously enabled pins
 *			which are not specified in the @arg remain enabled.
 *
	...
	switch (cmd) {
		...
		case PCIE215_IOCTL_IRQ_ENABLE:
			...
			if (arg) {
				...
				spin_unlock_irqrestore(&pcie215dev->lock,
						       flags);
			} else {
...
static void pcie215_config_print(struct pcie215 *pcie215dev)
{
	...
	dev_info(&pdev->dev, "Attached pcie215 to PCI [%s]: interrupt [%ld]"
			" pin [%u] major [%u]", pci_name(pdev),
			pcie215dev->irq, pin, pcie215dev->major);

	if (pcie215dev->ba_iomem)
		dev_info(&pdev->dev, "Base Address Configuration Register "
				"[%p] (I/O memory)", pcie215dev->ba_iomem);
	else
		dev_info(&pdev->dev, "Base Address Configuration Register "
				"[%08lx]", pcie215dev->ba);

	if (pcie215dev->lcr_iomem)
		dev_info(&pdev->dev, "Local Bus Configuration Register "
				"[%p] (I/O memory)", pcie215dev->lcr_iomem);
	else
		dev_info(&pdev->dev, "Local Bus Configuration Register "
				"[%08lx]", pcie215dev->lcr);
...
static int pcie215_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (!pci_find_capability(pdev, PCI_CAP_ID_PM)) {
		dev_err(&pdev->dev, "%s: No support for power managemet "
				"on this device\n", __func__);
	}
...
static int pcie215_suspend(struct device *dev)
{
	...
	/*
	 * Let the PCI subsystem do the rest (save the PCI standard configuration
	 * registers, prepare it for system wakeup (if necessary), and to put it
...
static int pcie215_resume(struct device *dev)
{
	...
	/*
	 * Let the PCI subsystem do the rest (restoring PCI standard registers, etc).
	 */
